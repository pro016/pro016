...
Como refuerzo para comprender los conceptos que hemos aprendido hasta el momento vamos a examinar algunos de ellos en un contexto diferente al de la máquina expendedora pero familiar. 
...
La clase contiene tres atributos: `nombre`, `numeroMatricula` y `edad`. Cada uno de ellos se inicializa en el único constructor existente. Los valores iniciales de todos ellos se establecen a partir de los parámetros pasados al constructor. El único atributo que dispone de un método getter y de un método setter asociado es el atributo `nombre`. Esto quiere decir que, de momento, no tenemos posibilidad de cambiar de ninguna forma los valores de los atributos `edad` y `numeroMatricula` una vez fijados inicialmente. En este caso, decimos que el valor de estos dos atributos son *inmutables*. En ocasiones vamos a hacer que todos los atributos de un objeto sean inmutables después de haberlo construido; la clase `String` va a ser un ejemplo importante de este tipo de caso.
...
En BlueJ crea un nuevo alumno cuyo `nombre` sea `“Belen Esteban García”` y cuyo `numeroMatricula` sea `”4502”` (indica al edad que desees). Comprueba que resultado de invocar el método `getNombreUsuarioGithub` sobre este objeto es `”Bel4502”’’.
